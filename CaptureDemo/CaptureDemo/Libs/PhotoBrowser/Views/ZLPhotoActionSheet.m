...
- (IBAction)btnPhotoLibrary_Click:(id)sender
{
    if (![self judgeIsHavePhotoAblumAuthority]) {
        ...
        photoBrowser.seleteOnlyOne = self.seleteOnlyOne;
        photoBrowser.mediaType = self.mediaType;
        photoBrowser.isCircular = self.isCircular;
        photoBrowser.canEdit = self.canEdit;

        weakify(self);
        __weak typeof(photoBrowser) weakPB = photoBrowser;
        [photoBrowser setEditCompletion:^(UIImage *seletedImage,NSString *filePath) {
            strongify(weakSelf);
            __strong typeof(weakPB) strongPB = weakPB;
            [strongSelf hide];

            [strongPB.navigationController dismissViewControllerAnimated:YES completion:nil];

            if (strongSelf.editCompletion) {
                if (seletedImage) {
                    strongSelf.editCompletion(seletedImage,nil);
                    return ;
                }
                if (filePath) {
                    strongSelf.editCompletion(nil,filePath);
                    return ;
                }
            }
        }];
...
- (void)changeBtnCameraTitle
{
    if (self.arraySelectPhotos.count > 0) {
        ...
        [self.btnCamera setTitle:GetLocalLanguageTextValue(ZLPhotoBrowserCameraText) forState:UIControlStateNormal];
        [self.btnCamera setTitleColor:kBlackColor forState:UIControlStateNormal];
    }
}
...
- (void)presentVC:(UIViewController *)vc
{
    ...
    [nav.navigationBar setTitleTextAttributes:@{NSForegroundColorAttributeName: kBlackColor}];
